const Blogs = () => {
  return (
    <div className="col-md offset-md-5 side-column">
      <section className="articles" id="article">
        <h4 className="name gray">ARTICLES.</h4>
        <div className="mt-4">
          <span className="date text-uppercase">June 30, 2020</span>
          <p className="mt-0">
            <a
              target="_blank"
              rel="noopener noreferrer"
              href="https://medium.com/@ahmadchata/align-and-justify-what-you-need-to-know-748f7f2629c8"
              className="text-decoration-none fs-6 link"
            >
              Align and Justify, What you need to know
            </a>
          </p>
          <span className="date text-uppercase">Nov 20, 2020</span>
          <p>
            <a
              target="_blank"
              rel="noopener noreferrer"
              href="https://www.microverse.org/blog/how-to-get-better-at-solving-coding-challenges"
              className="text-decoration-none fs-6 link"
            >
              How to Get Better At Solving Coding Challenges
            </a>
          </p>
          <span className="date text-uppercase">Dec 21, 2020</span>
          <p>
            <a
              target="_blank"
              rel="noopener noreferrer"
              href="https://www.microverse.org/blog/how-to-write-a-sql-query-in-ruby-on-rails"
              className="text-decoration-none fs-6 link"
            >
              How to Write a SQL Query in Ruby on Rails
            </a>
          </p>
        </div>
      </section>
    </div>
  );
};

export default Blogs;
